...
真正的 Xilinx sync word 是 `0xAA995566`（Xilinx UG470: 7 Series FPGAs Configuration User Guide）。在 `.bit` 裡以 big-endian 存成 bytes `AA 99 55 66`；經過 `swap32` 後在 `.bin` 裡變成 bytes `66 55 99 AA`；ARM 以 little-endian 讀這 4 個 bytes，得到 uint32 值 `0xAA995566`，正確。
